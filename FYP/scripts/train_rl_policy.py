...
The trained PPO weights are saved to models/rl/ppo_routing_latest.zip and
...
def main():
    ...
    # Wrap environment in Monitor for EvalCallback
    env = Monitor(env)
    
    # EvalCallback prevents overfitting by periodically evaluating the model
    # and saving the absolute best performing version.
    eval_callback = EvalCallback(
        env,
        best_model_save_path='./models/rl/',
        log_path='./data/logs/telemetry/',
        eval_freq=5000,
        deterministic=True,
        render=False,
    )
    ...
    t0 = time.time()
    pod.train(env, n_steps=args.steps, callbacks=[eval_callback])
    elapsed = time.time() - t0
    logger.info(f"Training complete in {elapsed:.1f}s ({args.steps/elapsed:.0f} steps/sec)")
    
    # Ensure the best model is used for the simulation
    best_model_path = Path('./models/rl/best_model.zip')
    latest_model_path = Path('./models/rl/ppo_routing_latest.zip')
    
    if best_model_path.exists():
        logger.info("Overfitting prevented: Restoring best_model.zip to ppo_routing_latest.zip")
        shutil.copy(best_model_path, latest_model_path)
    else:
        logger.info("Checkpoint saved to models/rl/ppo_routing_latest.zip")
