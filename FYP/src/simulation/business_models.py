...
class InventoryModel:
    ...
    def calculate_reorder_point(self, lead_time_minutes: float = None) -> float:
        """Reorder point s = predicted demand during lead time + safety stock."""
        lt = lead_time_minutes if lead_time_minutes is not None else self.lead_time_mean
        predicted_rate_kg_h = self.ai_forecaster.predict(self.demand_history_full)
        predicted_demand_kg = predicted_rate_kg_h * (lt / 60.0)
        protection_period = lt + self.review_period
    ...
    def calculate_order_up_to_level(self, lead_time_minutes: float = None) -> float:
        """Order-up-to level S using EOQ or fixed-days method."""
        if self.order_method == "EOQ":
            return self._eoq_level(lead_time_minutes)
        return self._fixed_days_level(lead_time_minutes)

    def _eoq_level(self, lead_time_minutes: float = None) -> float:
        predicted_rate_kg_h = self.ai_forecaster.predict(self.demand_history_full)
        ...
        if annual_demand > 0 and self.holding_cost > 0:
            ...
            s = self.calculate_reorder_point(lead_time_minutes)
            return max(self.min_order_qty, min(self.max_order_qty, s + eoq))
        return self._fixed_days_level(lead_time_minutes)

    def _fixed_days_level(self, lead_time_minutes: float = None) -> float:
        lt = lead_time_minutes if lead_time_minutes is not None else self.lead_time_mean
        predicted_rate_kg_h = self.ai_forecaster.predict(self.demand_history_full)
    ...
    def should_reorder(self, current_inventory: float, lead_time_minutes: float = None) -> bool:
        """True if inventory has fallen below the reorder point."""
        return current_inventory <= self.calculate_reorder_point(lead_time_minutes)

    def calculate_order_quantity(self, current_inventory: float, lead_time_minutes: float = None) -> float:
        """Quantity to order to bring inventory up to S."""
        order_qty = self.calculate_order_up_to_level(lead_time_minutes) - current_inventory
