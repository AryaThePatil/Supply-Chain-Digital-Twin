...
class RetailerAgent:
    ...
    def _check_inventory(self, current_time: float, engine=None) -> Optional[Dict]:
        ...
        # 1. Select best warehouse first (to predict lead time)
        selected_warehouse = self.select_warehouse(engine.warehouses) if engine else None
        
        # 2. Query AI ETA Forecaster if available
        lead_time_override = None
        if selected_warehouse and self.ai_manager and hasattr(self.ai_manager, 'prediction_pod') and self.ai_manager.prediction_pod.eta:
            try:
                dist = self.road_network._haversine_distance(
                    self.location[0], self.location[1],
                    selected_warehouse.location[0], selected_warehouse.location[1]
                )
                eta_pred = self.ai_manager.prediction_pod.eta.predict(dist)
                if eta_pred > 0:
                    lead_time_override = float(eta_pred)
            except Exception:
                pass
                
        # 3. Check if should reorder using dynamic AI lead time
        if self.inventory_model.should_reorder(self.current_inventory_kg, lead_time_override):
            # Calculate order quantity
            order_qty = self.inventory_model.calculate_order_quantity(self.current_inventory_kg, lead_time_override)
