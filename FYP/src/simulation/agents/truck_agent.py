...
class TruckAgent:
    ...
    def __init__(self, truck: Truck, road_network: RoadNetwork, 
        ...
        # State tracking for AI strategy locks
            ...
            if distance_available_km >= segment_remaining_km:
                ...
                if self.ai_manager is not None and time_spent_on_segment > 0:
                    self.ai_manager.record_segment_traversal(
                        segment, time_spent_on_segment, current_time, self.current_weather
                    )
    ...
    def _apply_strategy_action(self, action: int, current_time: float, reason: str = 'ai') -> bool:
        ...
        # Prevent thrashing: if the strategy hasn't changed and it's not an emergency,
        # do not throw away the current route and recalculate it.
        if action == self._last_strategy_action and reason != 'edge_emergency':
            return False
            
        self._last_strategy_action = action
    ...
    def _check_rerouting(self, current_time: float):
        ...
        if (self.ai_manager is not None
            ...
            # Learned ETA: accounts for real traffic patterns at this time and zone
            current_route_cost = self.ai_manager.prediction_pod.eta.predict_route(
                self.truck.current_route, current_time, weather,
