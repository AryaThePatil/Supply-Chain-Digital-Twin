...
class ETAForecaster:
    ...
    _ZONE_ENC: Dict[str, float] = {
        "RESIDENTIAL": 0.0, "HIGHWAY": 0.3, "OFFICE": 0.7, "SHOPPING": 1.0,
    }
    ...
    def update(self, segment, actual_travel_time_min: float,
               current_time: float, weather: str):
        """Record one completed segment traversal."""
        x = self._featurize(segment, current_time, weather)
        self._A += np.outer(x, x)
        self._b += x * actual_travel_time_min
        self._n_samples += 1
        self._weights = None  # invalidate cached solution

    def predict_segment(self, segment, current_time: float, weather: str) -> float:
    ...
    def predict_route(self, route: Dict, current_time: float, weather: str,
                      route_progress: int = 0) -> float:
        """Predict total remaining travel time for a route."""
        segments = route.get("segment_objects", [])
        return sum(
            self.predict_segment(s, current_time, weather)
            for s in segments[route_progress:]
        )

    def _featurize(self, segment, current_time: float, weather: str) -> np.ndarray:
        hour = (current_time / 60.0) % 24.0
        day_of_week = (current_time / (60.0 * 24.0)) % 7.0
        
        hour_rad = hour * (2.0 * math.pi / 24.0)
        day_rad = day_of_week * (2.0 * math.pi / 7.0)
        
        weather_enc = self._WEATHER_ENC.get(weather, 0.5)
        road_enc = self._ROAD_ENC.get(getattr(segment, "road_type", "secondary"), 0.5)
        
        zone = getattr(segment, "zone_type", "RESIDENTIAL")
        zone_enc = self._ZONE_ENC.get(zone, 0.0)
        
        density = getattr(segment, "current_traffic_density", 0.0) / 100.0
        return np.array([
            math.sin(hour_rad), math.cos(hour_rad),
            math.sin(day_rad), math.cos(day_rad),
            weather_enc, min(1.0, density), road_enc, zone_enc
        ], dtype=np.float64)
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            # Prevent shape mismatch crash if upgrading feature vector
            if data["A"].shape[0] != self._n_features:
                logger.warning(f"[ETAForecaster] Checkpoint feature size mismatch ({data['A'].shape[0]} vs {self._n_features}). Starting fresh.")
                return False
...
class PredictionPod:
    ...
    def record_segment_traversal(self, segment, actual_minutes: float,
        ...
        if self.eta is not None:
            self.eta.update(segment, actual_minutes, current_time, weather)
